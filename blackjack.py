...
def _deal_card(frame):
    ...
    # now we return cards face value
    return next_card
...
def score_hand(hand):
    ...
    for next_card in hand:
        ...
        if card_value == 1 and not ace:
            ...
            card_value = 11
        score += card_value
        # if we would be bust, check if there is an ace and subtract 10
        if score > 21 and ace:
            score -= 10
            ace = False
    return score
...
def new_game():
    ...
    # embedded frame to hol the card images
    player_card_frame = tkinter.Frame(card_frame, background="green")
    player_card_frame.grid(row=2, column=1, sticky="ew", rowspan=2)

    result_text.set("")

    # create the list to store the dealer's and players hands
    dealer_hand = []
    player_hand = []
